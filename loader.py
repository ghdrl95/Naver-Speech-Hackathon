...
def Mel_S(wav_file):
    ...
    input_nfft = N_FFT
    window_length = int(0.030 * SAMPLE_RATE)
    input_stride = int(round(SAMPLE_RATE*frame_stride))
    #logger.info("make melspectrogram start")
    S = librosa.feature.melspectrogram(y=y, sr = 16000, n_mels=128, n_fft=input_nfft,win_length=window_length, hop_length=input_stride)

    #log-Attitude MelSpectrogram 적용
    ...
    feat = torch.FloatTensor(norm_S)

    #MFCC 적용
    #log_S = librosa.power_to_db(S, ref=np.max)
    #mfcc = librosa.feature.mfcc(S=log_S,n_mfcc=39)
    #delta2_mfcc = librosa.feature.delta(mfcc, order=2)
    # feat = torch.FloatTensor(delta2_mfcc)

    feat = torch.FloatTensor(feat).transpose(0, 1)

    return feat
#wav파일을 읽어서 spectrogram 만드는 함수
def get_spectrogram_feature(filepath):
    '''
    (rate, width, sig) = wavio.readwav(filepath)
    sig = sig.ravel()
    #FFT/Frames에 대한 STFT(Short-Time Furier Transform) 생성
